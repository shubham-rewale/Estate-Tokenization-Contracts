...
const votingPeriod = 100;
    ...
    beforeEach(async () => {
    [mogulPlatformOwnerAddress,propertyManager,propertyOwner1, propertyOwner2, propertyOwner3,unauthorizedUserAddress,tenant, ...addrs]=await ethers.getSigners();
    ...
    DAOContract = await ethers.getContractFactory(
        "DAO"
      );
...
[token.address,maintenanceReserve.address,vacancyReserve.address,propertyManager.address],
  ...
  //set the maintenanceReserve contract address in the DAO contract
    await daoContract.connect(mogulPlatformOwnerAddress).setMaintenanceReserveAddress(maintenanceReserve.address);
  //set the vacancyReserve contract address in the DAO contract
  await daoContract.connect(mogulPlatformOwnerAddress).setVacancyReserveAddress(vacancyReserve.address);
  ...
  await rentalProperties.connect(propertyManager).enterRentalPropertyDetails(0,ethers.utils.parseEther("0.5"),ethers.utils.parseEther("0.5"));
  await rentalProperties.connect(propertyManager).initiateRentalPeriod(0,tenant.address,10,ethers.utils.parseEther("0.5"),ethers.utils.parseEther("0.5"),{ value: ethers.utils.parseEther("2")});
  });
  describe("testing of owner function",function(){
    it("Deployer should be the owner of the DAO contract", async () => {
        expect(await daoContract.owner()).to.equal(mogulPlatformOwnerAddress.address);
      });
  })
  describe("testing of owner function",function(){
    it("Deployer should be the owner of the DAO contract", async () => {
