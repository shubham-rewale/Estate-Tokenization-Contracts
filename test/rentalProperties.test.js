...
describe("Test Cases For Property Rental in Estate Tokenisation", function () {
  ...
  beforeEach(async () => {
    [mogulPlatformOwnerAddress, propertyOwner1, propertyOwner2, propertyOwner3,propertyManager,tenant, ...addrs] =
...
[token.address,maintenanceReserve.address,vacancyReserve.address,propertyManager.address],
  ...
  //initialize rental period
  // distributeRentAmount
  });
  ...
  //       await expect(rentalProperties.connect(propertyOwner1).enterRentalPropertyDetails(0,500,500)).to.be.revertedWith("Caller is not the Property Manager");
  //   });
  //   it("property manager should not be able to enter the details of rental property for a non existing property",async()=>{
  //       await expect(rentalProperties.connect(propertyManager).enterRentalPropertyDetails(1,500,500)).to.be.revertedWith("Property with the given Token Id does not exist");
  //   });
    
  //   it("property manager should not be able to enter the details of already registered rental property",async()=>{
  //       await rentalProperties.connect(propertyManager).enterRentalPropertyDetails(0,500,500);
  //       await expect(rentalProperties.connect(propertyManager).enterRentalPropertyDetails(0,500,500)).to.be.revertedWith("Property already registered for renting");
  //   });
  //   it("Property Maintenance Reserve should be more than zero",async()=>{
  //       await expect(rentalProperties.connect(propertyManager).enterRentalPropertyDetails(0,0,500)).to.be.revertedWith("Property Maintenance Reserve should be more than zero");
  //   });
  //   it("Property vacancy Reserve should be more than zero",async()=>{
  //       await expect(rentalProperties.connect(propertyManager).enterRentalPropertyDetails(0,500,0)).to.be.revertedWith("Property Vacancy Reserve should be more than zero");
  //   });
  // });
  ...
  //         await rentalProperties.connect(propertyManager).enterRentalPropertyDetails(0,500,500);
  //         await expect(rentalProperties.connect(propertyManager).updateReserveCapAmount(0,0,600)).to.emit(vacancyReserve,'VacancyReserveCapUpdated').withArgs(0,600);   
  //     });
  //   it("user other than property manager should not be able to update ReserveCap ",async()=>{
  //        await rentalProperties.connect(propertyManager).enterRentalPropertyDetails(0,500,500);
  //        await expect(rentalProperties.connect(propertyOwner1).updateReserveCapAmount(0,0,600)).to.be.revertedWith("Caller is not the Property Manager");
  //   });
  //   it("Only property manager should be able to call this function:testing of MaintenanceReserveCapUpdated",async()=>{
  //       await rentalProperties.connect(propertyManager).enterRentalPropertyDetails(0,500,500);
  //       // console.log( await maintenanceReserve.checkMaintenanceReserve(0));
  //        await expect(rentalProperties.connect(propertyManager).updateReserveCapAmount(0,600,0)).to.emit(maintenanceReserve,'MaintenanceReserveCapUpdated').withArgs(0,600);
  //       //  console.log( await maintenanceReserve.checkMaintenanceReserve(0));
  //   });
  // });
...
//           await rentalProperties.connect(propertyManager).enterRentalPropertyDetails(0,500,500);
//           await expect(rentalProperties.connect(propertyManager).initiateRentalPeriod(0,tenant.address,10,250,250, { value: 1000 })).to.emit(rentalProperties,'RentalPeriodInitiated').withArgs(0,tenant.address,await time.latest(),10);    
//       });
// it("user other than property manager should not be able to call this function:initiateRentalPeriod",async()=>{
//           await rentalProperties.connect(propertyManager).enterRentalPropertyDetails(0,500,500);
//           await expect(rentalProperties.connect(propertyOwner1).initiateRentalPeriod(0,tenant.address,10,250,250, { value: 1000 })).to.be.revertedWith("Caller is not the Property Manager");
//       });
//   it("property manager should not be able to initiate rental period for a non listing property",async()=>{
//      await expect(rentalProperties.connect(propertyManager).initiateRentalPeriod(1,tenant.address,10,250,250)).to.be.revertedWith("Property is not listed for the rental period initiation");
//   });
...
//   await rentalProperties.connect(propertyManager).enterRentalPropertyDetails(0,500,500);
//   await expect(rentalProperties.connect(propertyManager).initiateRentalPeriod(0,ethers.constants.AddressZero,10,250,250, { value: 1000 })).to.be.revertedWith("Provide Valid Tenant address");
// });
// it("property manager should not be able to initiate rental period if msg.value is zero ",async()=>{
//   await rentalProperties.connect(propertyManager).enterRentalPropertyDetails(0,500,500);
//   await expect(rentalProperties.connect(propertyManager).initiateRentalPeriod(0,tenant.address,10,250,250, { value: 0 })).to.be.revertedWith("Please deposite rent for the whole month while initiating rental period");
// });
...
//      await expect(rentalProperties.connect(propertyManager).distributeRentAmount(0,[propertyOwner1.address,propertyOwner2.address,propertyOwner3.address])).to.be.revertedWith("Property is not listed for the rental process");
//   });
//   it("property manager should not be able to distributeRentAmount for a non occupied property",async()=>{
//     await rentalProperties.connect(propertyManager).enterRentalPropertyDetails(0,500,500);
//     await expect(rentalProperties.connect(propertyManager).distributeRentAmount(0,[propertyOwner1.address,propertyOwner2.address,propertyOwner3.address])).to.be.revertedWith("Property is not currently occupied");
//  });
//  //incomplete....We need to make condition false
// //  it("property manager should not be able to distributeRentAmount if it doesn't have enough deposits to distribute the rent",async()=>{
// //   await rentalProperties.connect(propertyManager).enterRentalPropertyDetails(0,500,500);
// //   await expect(rentalProperties.connect(propertyManager).initiateRentalPeriod(0,tenant.address,10,250,250, { value: 1000 }))
// //   await expect(rentalProperties.connect(propertyManager).distributeRentAmount(0,[propertyOwner1.address,propertyOwner2.address,propertyOwner3.address])).to.be.revertedWith("Not enough deposits to distribute the rent");
// // });
// it("property manager should not be able to call this function : testing of distributeRentAmount",async()=>{
//   await rentalProperties.connect(propertyManager).enterRentalPropertyDetails(0,500,500);
//   // await expect(rentalProperties.connect(propertyManager).initiateRentalPeriod(0,tenant.address,10,250,150, { value: 400 }))
//   await expect(rentalProperties.connect(propertyManager).initiateRentalPeriod(0,tenant.address,10,250,250, { value: 10000 }))
//   // await expect(rentalProperties.connect(propertyManager).distributeRentAmount(0,[propertyOwner1.address,propertyOwner2.address,propertyOwner3.address])).to.emit(rentalProperties,'RentDistributed').withArgs(0,[propertyOwner1.address,propertyOwner2.address,propertyOwner3.address],[0,0,0]);  
...
//   });
  ...
  //     await rentalProperties.connect(propertyManager).enterRentalPropertyDetails(0,500,500);
  //     await expect(rentalProperties.connect(propertyManager).initiateRentalPeriod(0,tenant.address,10,250,250, { value: 1000 }))
  //     await expect(rentalProperties.connect(propertyManager).terminateRentalPeriod(0)).to.emit(rentalProperties,'RentalPeriodTerminated').withArgs(0,tenant.address,await time.latest()+2,500);
  //   });
  ...
  //     await expect(rentalProperties.connect(propertyManager).terminateRentalPeriod(0)).to.be.revertedWith("Property is not listed for the rental process");
  //   });
  //   it("property manager should not be able to terminateRentalPeriod of a non occupied property",async()=>{
  //     await rentalProperties.connect(propertyManager).enterRentalPropertyDetails(0,500,500);
  //     await expect(rentalProperties.connect(propertyManager).terminateRentalPeriod(0)).to.be.revertedWith("Property is already vacant");
  //   });
